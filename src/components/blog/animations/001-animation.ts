export const Animation001 = {
    ...
    return `
      <div class="blog-animation-card rounded-xl border border-ctp-surface0 bg-ctp-mantle/70 overflow-hidden shadow-xs font-mono">
        <div class="p-4 sm:p-5 space-y-4">
          ...
          <div class="p-3.5 rounded bg-ctp-crust border border-ctp-surface0/40 leading-relaxed text-sm tracking-wide whitespace-pre-wrap break-words select-none font-mono" id="anim1-haystack"></div>
          ...
          <div class="p-3 rounded bg-ctp-crust/90 text-xs border border-ctp-surface0/40 space-y-1 font-mono">
            ...
            <div id="anim1-log" class="text-xs text-ctp-text break-words">
  ...
  init(wrapper: Element): void {
    ...
    const runBtn = wrapper.querySelector('#anim1-run') as HTMLButtonElement | null;
    ...
    const tick = () => {
      ...
      if (isMatch) {
        stopScan();
        if (logEl) {
          logEl.innerHTML = `<span class="text-ctp-green font-semibold">haystack[+${currentIndex}] (${displayChar}) === ${displayNeedle} → MATCH FOUND</span>`;
        }
      } else {
        if (logEl) {
          logEl.innerHTML = `<span class="text-ctp-subtext0">haystack[+${currentIndex}] (${displayChar}) !== ${displayNeedle} → 0 matches in 1 byte (i += 1)</span>`;
        }
        currentIndex++;
      }
    };
